...
//transaction.js delegate to Input.fromBufferReader() to process each transaction's input
//https://bitcoin.org/en/developer-reference#txin
Input.fromBufferReader = function(br) {
  //console.debug(`YCM TRACKER === bitcore-lib/lib/transaction/input/input.js fromBufferReader()`);
  var input = new Input();
  //an input's previous_output is basically a tx id hash (32bytes) plus otuput index (4bytes)
  //read 32 bytes
  input.prevTxId = br.readReverse(32);
  //read 4 bytes
  input.outputIndex = br.readUInt32LE();
  //first read the size, then read the next xxx bytes from the buffer accordingly
  input._scriptBuffer = br.readVarLengthBuffer();
  //read last 4 bytes - sequence number
