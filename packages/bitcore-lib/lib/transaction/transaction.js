...
Transaction.prototype.inspect = function() {
  ...
  return '<YCM-Inspect-Tx-Begin <Transaction: '
          + ' input size: ' + this.inputs.length 
          + ' output size: ' + this.outputs.length 
          + ' inputs details: ' + util.inspect(this.inputs)
          + ' outputs details: ' + util.inspect(this.outputs)
          + ' YCM-Inspect-Tx-End> ';
};
...
//reader is bitcore-lib/lib/encoding/bufferreader.js where it defines most of the read func
Transaction.prototype.fromBufferReader = function(reader) {
  //console.debug(`YCM TRACKER === bitcore-lib/lib/transaction/transaction.js fromBufferReader()`);
  $.checkArgument(!reader.finished(), 'No transaction data received');

  this.version = reader.readInt32LE();
  var sizeTxIns = reader.readVarintNum();

  //TODO low: watch https://www.youtube.com/watch?v=DzBAG2Jp4bg&feature=youtu.be&t=1m8s to understand the diff between segwit and non-segwit
  // check for segwit
  ...
  //we get "Input" package to process the next x inputs
  //and put all processed inputs to this.inputs
  for (var i = 0; i < sizeTxIns; i++) {
    var input = Input.fromBufferReader(reader);
    this.inputs.push(input);
  }

  //similarly we delegate to "Output" to process outputs
  var sizeTxOuts = reader.readVarintNum();
  for (var j = 0; j < sizeTxOuts; j++) {
    this.outputs.push(Output.fromBufferReader(reader));
  }

  //TODO low: where can i find this segwit data structure from https://bitcoin.org/en/developer-reference?
  if (hasWitnesses) {
  ...
  //the last 4 bytes of a raw transaction is the lock time
