...
export class InternalStateProvider implements CSP.IChainStateService {
  ...
  async getLocalTip({ chain, network }) {
    //go query the block collection from mongodb
    //sort by height, in descending order, take the first block i.e. the most recent block
    return BlockModel.collection.findOne({ chain, network, processed: true }, { sort: { height: -1 } });
  }

  //given chain+network, find all blocks with processed=true, sort by height, in decending order, take the first 30 blocks
  //form a block -> blockHash map and return 
  async getLocatorHashes(params) {
    ...
    logger.debug(`YCM internal.ts getLocatorHashes ${locatorBlocks.length}`);
    //give the 30 blocks in array form, we take the block and its hash, form a map, and return the map
