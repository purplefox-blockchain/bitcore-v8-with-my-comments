...
function Pool(options) {
  ...
  //the pool constructor is called from p2p.ts with dnsSeed=false, listenAddr=false

  //whatever addr in bitcore.config.json are added first,
  //later in connect() we will add addrs returned by dnsseed to the list as well, provided that dnsSeed flag is true
  if (options.addrs) {
  ...
  //if the pool has 'listen' func enabled, we will react to peeraddr event and call peerAddrEvent
  if (this.listenAddr) {
  ...
  //we register for 'seed' event
  //of course if we dont have dnsSeed=true, we won't get this event. TO CONFIRM
  //my understanding is, this is a 'seed' event sent by the dnsseed server
  //the reply consists of a list of ips, for each ip, we _addAddr to our list
  ...
  //if there is a peer disconnect from us
  //deprioritize that addr from our list
  //mark it as disconnected
  //connect to as many as peers until we reach the cap
...
Pool.prototype._fillConnections = function _fillConnections() {
  ...
  for (var i = 0; i < length; i++) {
    //before we reconnect this addr, we see if we reach the max allowed connected peer
    if (this.numberConnected() >= this.maxSize) {
      break;
    }
    //since max is not reached, we try to connect to this peer
...
Pool.prototype._connectPeer = function _connectPeer(addr) {
  ...
  if (!this._connectedPeers[addr.hash]) {
    ...
    //if we are notified by this peer we are connected,
    //then we (the pool) fire an internal event 'peerconnect'
    //whoever subscribe to the pool's peerconnect event will get notified
...
//we need to prepare the pool to handle future ready/disconnect events
//so that, in the future,
//when this peer is ready, the pool fire a 'peerready' to its own subscribers
//when this peer is disconnected, the pool fire a 'peerdisconnect' to its own subscribers
//when there is any event being received by this peer, the pool fire a 'peerxxxx' to its own subscribers
//at the end of this, no matter what happen to any peer in our pool, we will get notified and we will notify our subscribers
/**
...
Pool.prototype._addPeerEventHandlers = function(peer, addr) {
  ...
  //if this peer is ready, we (the pool) notify all our subscribers by firing a 'peerready' event
  peer.on('ready', function peerReady() {
    self.emit('peerready', peer, addr);
  });
  //for all other events, we prefix a 'peer' string and fire the event to our own subscribers
  //why we want to separate disconnect/ready events from this forEach loop? because the last arg in emit is different
  Pool.PeerEvents.forEach(function addPeerEvents(event) {
    peer.on(event, function peerEvent(message) {
      //this 'message' is already parsed, the parsing is done in peer.js
      //console.debug(`YCM bitcore-p2p/pool.js on peer event ${event} message: ${util.inspect(message)}`);
...
//this listen() is not called by any at the moment
//according to the doc, by calling this listen function, the pool will listen to incoming socket connections to add peers to the pool.
/**
